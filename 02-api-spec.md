...
Game-art manifest: every location, monster type, NPC, and item mapped to
its repo-relative SVG path (`assets/...`), generated by
`scripts/make_assets.py` from `assets.json` and served statically under
`/assets` (e.g. `GET /assets/manifest.json`,
`GET /assets/locations/oakhollow_forest.svg`).

```json
{
  "locations": { "riverside_village": "assets/locations/riverside_village.svg" },
  "monsters": { "Forest Wolf": "assets/monsters/forest_wolf.svg" },
  "npcs": { "npc_scout": "assets/npcs/npc_scout.svg" },
  "items": { "itm_rat_pelt": "assets/items/itm_rat_pelt.svg" }
}
```

`GET /status` (`inventory[].asset`, `location_asset`),
`GET /world/here` (`asset`, plus per-NPC/monster/ground-item `asset`), and
`GET /world/state` (per-zone/per-entity `asset`) carry the same paths
inline so CLIs like `agent-starter/play.py` can show art every turn.
All `asset` fields are additive — old clients ignore them. Missing files
are fine: fall back to emoji/text, never fail the turn.
